...
void test_nfa_merge()
{
    ...
    regex::nfa keyword_nfa = regex::nfa::builder::build("if");
    regex::nfa identifier_nfa = regex::nfa::builder::build("variable");
    regex::nfa number_nfa = regex::nfa::builder::build("123");
    ...
    std::cout << "最终合并NFA (关键字|标识符|数字): " << final_nfa.to_string() << std::endl;
    
    // 将合并后的NFA转换为DFA
    regex::dfa combined_dfa(final_nfa);
    ...
    // 测试更复杂的合并
    std::cout << "\n--- 测试复杂合并 ---" << std::endl;
    regex::nfa abc_nfa = regex::nfa::builder::build("abc");
    regex::nfa xyz_nfa = regex::nfa::builder::build("xyz");
    regex::nfa num_nfa = regex::nfa::builder::build("[0-9]+");
    
    // 合并多个NFA
    regex::nfa complex_nfa = abc_nfa | xyz_nfa;
    complex_nfa = complex_nfa | num_nfa;
    
    regex::dfa complex_dfa(complex_nfa);
    ...
    // 验证每个单独的NFA仍然有效
    std::cout << "\n--- 验证原NFA未被修改 ---" << std::endl;
    regex::dfa original_keyword_dfa(keyword_nfa);
    regex::dfa original_identifier_dfa(identifier_nfa);
    regex::dfa original_number_dfa(number_nfa);
    
    std::cout << "原关键字NFA匹配'if': " << original_keyword_dfa.match("if") << std::endl;
    std::cout << "原关键字NFA匹配'variable': " << original_keyword_dfa.match("variable") << std::endl;
    std::cout << "原标识符NFA匹配'variable': " << original_identifier_dfa.match("variable") << std::endl;
    std::cout << "原标识符NFA匹配'if': " << original_identifier_dfa.match("if") << std::endl;
...
void test_merge_associativity()
{
    ...
    regex::nfa a_nfa = regex::nfa::builder::build("a");
    regex::nfa b_nfa = regex::nfa::builder::build("b");
    regex::nfa c_nfa = regex::nfa::builder::build("c");
    
    // 测试 (a | b) | c
    regex::nfa left_assoc = (a_nfa | b_nfa) | c_nfa;
    regex::dfa left_dfa(left_assoc);
    
    // 测试 a | (b | c)  
    regex::nfa right_assoc = a_nfa | (b_nfa | c_nfa);
    regex::dfa right_dfa(right_assoc);
...
int main()
{
    ...
    std::cout << "结论：NFA类现在支持直接合并操作，可以实现类似Flex的多规则并行匹配。" << std::endl;
